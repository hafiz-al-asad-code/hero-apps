...
const AppCard = ({ app }) => {
  ...
  return (
    <Link to={`/appDetails/${id}`} className='p-4 bg-white rounded-[4px] shadow-lg hover:scale-105'>
      <div>
        <img className='w-full h-[285px]' src={image} alt="" />
      </div>
      <h4 className='text-xl font-medium my-4'>{title}</h4>
      <div className='flex justify-between items-center'>
        <p className='font-medium flex items-center gap-2 py-1.5 px-2.5 rounded-[4px] bg-[#f1f5e8] text-[#00d390]'><span><Download className='w-4 h-4' /></span> <span>{downloads}M</span></p>
